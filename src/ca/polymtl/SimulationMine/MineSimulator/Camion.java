...
public abstract class Camion {
	...
	//etats possibles
	//
	/** �tat indiquant un camion auquel aucune activit� n'est assign�e.	 */
	public static int ETAT_INACTIF = 1;
	/** �tat indiquant un camion en route vers sa destination.	 */
	public static int ETAT_EN_ROUTE = 2;
	/** �tat indiquant un camion en attente de chargement.	 */
	public static int ETAT_ATTENTE = 3;
	/** �tat indiquant un camion en chargement.	 */
	public static int ETAT_EN_TRAITEMENT = 4;
	/** �tat indiquant un camion qui viens juste d'arriver � sa destination.	 */
	public static int ETAT_JUSTE_ARRIVE = 5;
	/** �tat indiquant un camion en d�chargement.	 */
	...
	// Station (null si � aucune station)
	//
	...
	/**
	 * 
	 * @return Le temps total d'attente du camion � la station
	 */
	...
	/**
	 * 
	 * @return La moyenne du temps d'attente du camion � la station donn�e
	 */
	...
	/**
	 * 
	 * @return Si le camion est pr�sentement en charge, retourne le temps courant de chargement.
	 */
	...
	/** 
	 * 
	 * @return Station courante du camion (ind�fini si le camion n'est pas � une station)
	 */
	...
	/**
	 * 
	 * @return Le nombre de fois o� le camion est all� � la station
	 */
	...
	/**
	 * 
	 * @return Le nombre de voyages (all�-retours) que le camion a effectu� durant la simulation courante.
	 */
	...
	/**
	 * 
	 * @return Destination du camion, ou null si le camion n'est pas en d�placement.
	 */
	...
	/**
	 * 
	 * @return Type de roche dans le camion (ind�fini si le camion est vide)
	 */
	...
	/**
	 * 
	 * @return �tat du camion correspondant � l'activit� en cours.
	 */
	public int getState() {
		return state;
	}
	...
	/**
	 * 
	 * @return Le temps total (s) que le camion a pass� en attente durant la simulation courante.
	 */
	 ...
	 * Utilis� pour l'interface graphique.
	 */
	...
	protected void advance(double temps) {
		...
		if(temps > this.getRemainingTimeInTurn()+0.0001) {
			throw new IllegalStateException("Le temps de deplacement ne doit pas d�passer le temps restant : "+temps + " > "+this.getRemainingTimeInTurn());
		}
	...
	protected void attend(double temps) {
		...
		if(temps > this.getRemainingTimeInTurn()+0.0001) {
			throw new IllegalArgumentException("Le temps d'attente ne doit pas d�passer le temps restant : "+temps+" > "+this.getRemainingTimeInTurn());
		}
	...
	protected void charge(double quantite, double temps) {
		...
		//marge d'erreur de 0.0001 pour erreur num�rique
		if(quantite - 0.0001 > this.getChargeMax()-this.charge) {
			throw new IllegalArgumentException("Quantit� charg�e trop grande : "+quantite+" > "+(this.getChargeMax()-this.charge));
		}
		if(temps > this.getRemainingTimeInTurn()+0.0001) {
			throw new IllegalArgumentException("Temps de d�charge trop grand : "+temps+" > "+this.getRemainingTimeInTurn());
		}
		if(this.getState()!= Camion.ETAT_EN_TRAITEMENT) {
			throw new IllegalStateException("Peut seulement d�charger en ETAT_CHARGE. �tat actuel : "+this.getState());
		}
	...
	protected void decharge(double quantiteDecharge, double tempsTraitement) {
		...
		//valide la demande de decharge
		//marge d'erreur de 0.0001 pour erreur num�riques
		if(quantiteDecharge - 0.0001 > this.charge) {
			throw new IllegalArgumentException("Quantit� d�charg�e trop grande : "+quantiteDecharge+" > "+this.charge);
		}
		if(tempsTraitement > this.getRemainingTimeInTurn()+0.0001) {
			throw new IllegalArgumentException("Temps de d�charge trop grand : "+tempsTraitement+" > "+this.getRemainingTimeInTurn());
		}
		if(this.getState()!= Camion.ETAT_EN_TRAITEMENT) {
			throw new IllegalStateException("Peut seulement d�charger en ETAT_EN_TRAITEMENT. �tat actuel : "+this.getState());
		}
	...
	//retourne true si l'iteration actuelle est terminee pour le camion (il a �puis� tout son temps)
	//
	...
	//fait attendre le camion jusqu'� la fin du tour
	//
	/*
