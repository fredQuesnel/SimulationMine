...
public class Mine {
	...
	public static class ExampleId{
		...
		private int id;
		private String name;
		public ExampleId(int id, String name){
			this.id = id;
			this.name = name;
		}
		public int getId() {
	...
	private static ArrayList<ExampleId> createExampleIds() {
		...
		exampleIds.add(new ExampleId(Mine.EXEMPLE1, "4 pelles"));
		exampleIds.add(new ExampleId(Mine.EXEMPLE2, "10 pelles"));
		return exampleIds;
	}
...
/*
	...
	*/
	...
	protected void init(int exempleNb, int nbCamions) {
		erasePreviousMine();
		
		//retrouve l'objet ExampleId de l'exemple
		ExampleId exId = null;
		for(int i = 0 ; i < this.exampleIds.size(); i++){
			System.out.println("Compare "+exempleNb+" "+exampleIds.get(i).getId());
			if(exempleNb == exampleIds.get(i).getId()){
		...
		if(exId == null){
			throw new IllegalArgumentException("numero d'exemple : "+exempleNb);
		}	
	
		this.name = exId.getName();
		
		this.exemple = exempleNb;
		if(exempleNb == Mine.EXEMPLE1) {
			initExemple1(nbCamions);	
		}
		else if(exempleNb == Mine.EXEMPLE2) {
			initExemple2(nbCamions);
		}
		else {
	...
	//
	private void initExemple1(int nbCamions) {
	...
	//cree l'instance de mine conforme a l'exemple 1
	//
	private void initExemple2(int nbCamions) {
